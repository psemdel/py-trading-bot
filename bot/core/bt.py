...
class BT(VBTfunc):
    ...
    def preselect_vol_sub(self, ii, **kwargs):
        ...
        for e in res:
            ...
            #strat11 need to be handled here, otherwise the chance to buy something is very low.
            if self.ent11[symbol_complex].values[ii] or kwargs.get("nostrat11",False):
...
class WQ(VBTfunc):
    ...
    def def_cand(self):
        ...
        for ii in range(len(self.out)):
            if self.trend.values[ii]<5: #block the mechanism if the trend is too bad on the index
                ind_max=np.argmax(self.out.iloc[ii].values)
                if self.nb in [1]:
