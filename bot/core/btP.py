...
### For production
#We try to mutualize the calculcation as much as possible to avoid re-calculating the same several times
...
#       long=kwargs.get("long",True)
       
#       if kwargs.get("PRD",False):
#           self.preselect_retard_sub(len(self.close.index)-1,long)   
#       else:
#           for ii in range(len(self.close.index)):
#               if self.hold_dur > 15:
#                   self.excluded.append(self.pf[0])
                
#               res=self.preselect_retard_sub(ii,long)                         
#               self.calculate_retard(ii,long,**kwargs)
