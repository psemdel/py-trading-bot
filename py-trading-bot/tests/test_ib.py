...
class TestIB(TestCase):
    ...
    def test_entry_order_manual(self):
        ...
        order=m.Order.objects.get(action=self.a2) #if not there, it will return an error
...
'''
#Those tests will produce real orders, be careful!!
    def test_entry_order_auto(self):
        ...
        order=m.Order.objects.get(action=self.a) #if not there, it will return an error
    ...
    def test_exit_order_manual(self):        
        ...
        ib.entry_order_sub(self.a,"none","Paris",False,True)   
        t=ib.exit_order_sub(self.a,"none","Paris",False,True)   
        self.assertTrue(t)
        self.assertEqual(len(pf.retrieve()),0)
        ocap=m.get_order_capital("none","Paris")
        self.assertEqual(ocap.capital,1)
